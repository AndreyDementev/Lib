...
#define UN_LOCK_TIME 200																	//моя правка

void DS3231::_burstRead()
{
	if (_use_hw)
	{
		uint32_t TimeIn ;																	//моя правка
		bool UnLock = false;																//моя правка
		if (_err_rtc) begin();																//моя правка
		...
		for (int i=0; i<6; i++)
		{
			TimeIn = millis();																//моя правка
			while (((twi->TWI_SR & TWI_SR_RXRDY) != TWI_SR_RXRDY) & !UnLock) {				//моя правка
				UnLock = (millis() - TimeIn) > UN_LOCK_TIME;								//моя правка
			}																				//моя правка
			if (UnLock) break;																//моя правка
			_burstArray[i] = twi->TWI_RHR;
		}
		
		if (UnLock){																		//моя правка
			_err_rtc = true;																//моя правка
			for(int i=0; i<6; i++) {_burstArray[i] = 0;}									//моя правка
			twi->TWI_CR = TWI_CR_STOP;														//моя правка
			return;																			//моя правка
		} 	   																				//моя правка
		else _err_rtc = false;																//моя правка
...
uint8_t DS3231::_readRegister(uint8_t reg)
{
	...
	if (_use_hw)
	{
		...
		TimeIn = millis();																	//моя правка
		while (((twi->TWI_SR & TWI_SR_TXCOMP) != TWI_SR_TXCOMP) & !UnLock) {				//моя правка
			UnLock = (millis() - TimeIn) > UN_LOCK_TIME;									//моя правка
		}		
	}
	else
...
void DS3231::_writeRegister(uint8_t reg, uint8_t value)
{
	...
	if (_use_hw)
	{
		...
		TimeIn = millis();																	//моя правка
		while (((twi->TWI_SR & TWI_SR_TXRDY) != TWI_SR_TXRDY) & !UnLock) {					//моя правка
			UnLock = (millis() - TimeIn) > UN_LOCK_TIME;									//моя правка
		}		
		// Send STOP condition
		twi->TWI_CR = TWI_CR_STOP;
